...
class PRProcessor:
    ...
    def __init__(self, config: Config, github_integration: Optional[GitHubIntegration] = None, notification_manager: Optional[NotificationManager] = None, llm_integration: Optional[Any] = None):
        ...
        if llm_integration is None and hasattr(config, 'openrouter') and hasattr(config.openrouter, 'api_key'):
            from integrations.openrouter_integration import OpenRouterIntegration
            llm_integration = OpenRouterIntegration(
                api_key=config.openrouter.api_key,
                model=getattr(config.openrouter, 'model', 'meta-llama/llama-3.1-70b-instruct')
            )
    ...
    async def process_pr(self, pr_info) -> PRProcessingResult:
        ...
        if isinstance(pr_info, int) or hasattr(pr_info, 'number'):
            if self.github_integration is None:
                logger.error("github_integration is required to fetch PR info but was not provided")
                return PRProcessingResult(
                    success=False,
                    message="GitHub integration not configured — cannot fetch PR info",
                    actions_taken=[],
                    pr_number=pr_info if isinstance(pr_info, int) else getattr(pr_info, 'number', 0)
                )
            pr_number_to_fetch = pr_info if isinstance(pr_info, int) else pr_info.number
            pr_data = self.github_integration.get_pr_info_dict(pr_number_to_fetch)
        else:
            pr_data = pr_info
        ...
        try:
            ...
            if review_result.get('suggested_changes'):
                if self.github_integration is None:
                    logger.error("github_integration is required to apply changes but was not provided")
                    return PRProcessingResult(
                        success=False,
                        message="GitHub integration not configured — cannot apply suggested changes",
                        actions_taken=[],
                        pr_number=pr_number
                    )
    ...
    async def review_pr_only(self, pr_info) -> PRProcessingResult:
        ...
        try:
            ...
            if review_result.get('should_comment', False):
                if self.github_integration is None:
                    logger.error("github_integration is required to add comments but was not provided")
                    return PRProcessingResult(
                        success=False,
                        message="GitHub integration not configured — cannot add review comment",
                        actions_taken=[],
                        pr_number=pr_number
                    )
                comment = review_result.get('comment', 'AI review completed')
                if comment:
                    comment_added = self.github_integration.add_comment(pr_number, comment)
                    if comment_added:
